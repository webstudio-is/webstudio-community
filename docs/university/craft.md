...
| Box, Slot, HTML Embed, and Collection |        | <ul><li>Always create a custom name.</li><li>For Boxes, avoid layout-oriented naming such as as <code>1/2</code> or <code>left</code>. Instead, use a name based on the content like <code>Image Wrapper</code>.</li></ul> | `Wrapper` `Card` `Animation Script`                                         | The purpose of these components are not clear without a custom name.                                       |
